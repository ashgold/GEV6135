...
def three_layer_convnet(x, params, hyperparams):
    ...
    x = F.relu(F.conv2d(x, conv_w1, conv_b1, padding=pad_size_1))
    x = F.relu(F.conv2d(x, conv_w2, conv_b2, padding=pad_size_2))
    x = x.flatten(1)
...
def initialize_three_layer_conv_part2():
    ...
    args = {
        'in_shape': (C,H,W),
        'conv_params': [
            (channel_1, kernel_size_1, pad_size_1),
            (channel_2, kernel_size_2, pad_size_2)
        ],
        'num_classes': num_classes
    }
    model = ThreeLayerConvNet(**args)
...
class PlainBlock(nn.Module):
    ...
    def __init__(self, Cin, Cout, downsample=False):
        ...
        #       You don't have give a specific name to modules.              #
        ######################################################################
        # Replace "pass" statement with your code
        self.net = nn.Sequential(
            nn.BatchNorm2d(Cin),
            nn.ReLU(),
            nn.Conv2d(Cin, Cout, 3, stride=2 if downsample else 1, padding=1, bias=False),
            nn.BatchNorm2d(Cout),
            nn.ReLU(),
    ...
    def __init__(self, Cin, Cout, downsample=False):
        ...
        if not downsample:
            if Cin == Cout:
                ...
                self.shortcut = nn.Conv2d(Cin, Cout, 1, 1, bias=False)        
        else:
            self.shortcut = nn.Conv2d(Cin, Cout, 1, 2, bias=False)
...
class ResNet(nn.Module):
    def __init__(self, stage_args, in_channels=3, block=ResidualBlock,
        ...
        # Replace "pass" statement with your code
        Cin = stage_args[0][0]
        blocks = [ResNetStem(in_channels, Cin)]
        for stage in stage_args:
            Cout, num_blocks, downsample = stage
            blocks.append(ResNetStage(Cin, *stage, block))
            Cin = stage[0] * block.expansion
...
class ResidualBottleneck(nn.Module):
    ...
    def __init__(self, Cin, Cout, downsample=False):
        ...
        self.block = nn.Sequential(
            nn.BatchNorm2d(Cin),
            nn.ReLU(),
            nn.Conv2d(Cin, Cout, 1, 2 if downsample else 1, bias=False),
            nn.BatchNorm2d(Cout),
            nn.ReLU(),
            nn.Conv2d(Cout, Cout, 3, padding=1, bias=False),
            nn.BatchNorm2d(Cout),
            nn.ReLU(),
            nn.Conv2d(Cout, Cout*4, 1, bias=False),
        )
        Cout *= 4
        self.shortcut = nn.Identity()
        if not downsample:
