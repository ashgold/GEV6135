...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for (i, _train) in enumerate(x_train):
        tri = torch.flatten(_train)
        for (j, _test) in enumerate(x_test):
            tej = torch.flatten(_test)
            dists[i,j] = (tej - tri).pow(2).sum().sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for (i, _train) in enumerate(x_train):
        tri = torch.flatten(_train)
        te = torch.flatten(x_test, start_dim=1)
        dists[i] = (te - tri).pow(2).sum(axis=1).sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # => sqrt(x1^2 - 2x1y1 + y1^2 + x2^2 - 2x2y2 + y2^2)
    # => sqrt(-2(x1y1 + x2y2) + (x1^2 + x2^2) + (y1^2 + y2^2))
    # => sqrt(-2(x dot y) + sum(x^2) + sum(y^2)
    tr = torch.flatten(x_train, start_dim=1)  #(num_train,)
    te = torch.flatten(x_test, start_dim=1) #(num_test,)
    mult = (tr @ te.T) * -2 #(num_train, num_test)
    sum_train = tr.pow(2).sum(axis=1)
    sum_test = te.pow(2).sum(axis=1)
    # (num_train, 1) + (num_train, num_test) + (1, num_test)
    dists = torch.sqrt(sum_train.unsqueeze(1) + mult + sum_test.unsqueeze(0))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _topk = torch.topk(dists.T, k, largest=False)
    y_topk = y_train[_topk.indices]
    # for every top-k y list in test samples
    for i, y in enumerate(y_topk):
        y_cnt = y.bincount()
        #argmax returns the first maximal value in the list.
        #"function SHOULD return the smallest label" condition works
        #Because index of y_cnt means labels and the first(smallest) maximal value is selected.
        y_pred[i] = y_cnt.argmax()
...
def knn_cross_validate(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_folds = list(x_train.chunk(chunks=num_folds))
    y_train_folds = list(y_train.chunk(chunks=num_folds))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    for i in range(num_folds):
        valid_index = i
        train_indices = [idx for idx in range(num_folds) if idx != i]
        _x_train = torch.cat([x_train_folds[idx] for idx in train_indices])
        _y_train = torch.cat([y_train_folds[idx] for idx in train_indices])
        _x_valid, _y_valid = x_train_folds[valid_index], y_train_folds[valid_index]
        classifier = KnnClassifier(_x_train, _y_train)
        for k in k_choices:
            acc = classifier.check_accuracy(_x_valid, _y_valid, k=k)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies:
        ...
        if mean_acc > best_k_acc:
            best_k, best_k_acc = k, mean_acc
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
